    ...
    - id: "ak_openai_001"           # API Key 唯一标识
      ...
      ban_rules:                    # 自动封禁规则（可选）
        - id: "rule_001"
          name: "高频错误封禁"
          condition:
            type: "ErrorRate"       # ErrorRate | RequestCount | ConsecutiveErrors
...
2. **路由级别** (`routes[].upstream.upstream_key_max_inflight`)
3. **全局级别** (`rate_limit.per_minute`, `concurrency.*`)
